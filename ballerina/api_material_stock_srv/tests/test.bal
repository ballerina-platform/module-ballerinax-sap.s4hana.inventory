...
import sap.s4hana.api_material_stock_srv.mock as _;
...
function testListA_MaterialStocks() returns error? {
    CollectionOfA_MaterialStockWrapper listMaterialStocks = check s4HanaClient->listA_MaterialStocks();
    test:assertTrue(listMaterialStocks.d?.results !is (), "The collection list of material stocks is expected to be non-empty.");
}

@test:Config {
}
function testListA_MaterialSerialNumber() returns error? {
    CollectionOfA_MaterialSerialNumberWrapper listMaterialSerialNumber = check s4HanaClient->listA_MaterialSerialNumbers();
    test:assertTrue(listMaterialSerialNumber.d?.results !is (), "The collection list of material serial numbers is expected to be non-empty.");
}
