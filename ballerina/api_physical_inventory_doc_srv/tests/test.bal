...
import sap.s4hana.api_physical_inventory_doc_srv.mock as _;
...
function testListA_PhysInventoryDocHeaders() returns error? {
    CollectionOfA_PhysInventoryDocHeaderWrapper listPhysInventoryDocHeaders = check s4HanaClient->listA_PhysInventoryDocHeaders();
    test:assertTrue(listPhysInventoryDocHeaders.d?.results !is (), "The collection list of physical inventory documents is expected to be non-empty.");
}

@test:Config {
}
function testListA_SerialNumberPhysInventoryDocs() returns error? {
    CollectionOfA_SerialNumberPhysInventoryDocWrapper listSerialNumberPhysInventoryDocs = check s4HanaClient->listA_SerialNumberPhysInventoryDocs();
    test:assertTrue(listSerialNumberPhysInventoryDocs.d?.results !is (), "The collection list of physical inventory document serial numbers is expected to be non-empty.");
}
